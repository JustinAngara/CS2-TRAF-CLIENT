...
{
	MessageBoxA(NULL, "DLL loaded! Setup starting...", "Asphyxia Debug", MB_OK);
...
BOOL APIENTRY CoreEntryPoint(HMODULE hModule, DWORD dwReason, LPVOID lpReserved)
{
	...
	if (dwReason == DLL_PROCESS_ATTACH)
	{
		...
		{
			// Write a simple file to prove DLL loaded
			FILE* test = fopen("C:\\dll_loaded_test.txt", "w");
			if (test)
			{
				fprintf(test, "DLL attached!");
				fclose(test);
			}
		}
